...
SeleniumServer.prototype.start = function (cb) {
  ...
  java.stderr.on('data', function (chunk) {
    that.emit('warn', chunk.toString('utf8'));
  });
};

SeleniumServer.prototype.stop = function (cb) {
  var that = this;
  var child = that._process;
  child.once('close', function () {
    that.emit('log', 'Selenium server has closed down');
    cb();
  });
  child.kill('SIGINT');
};
...
SeleniumServer.prototype._installChromeDriver = function (cb) {
  ...
  fs.exists(bin, function (exists) {
    if (exists) { return cb(); }
    that.emit('log', 'ChromeDriver is not installed.');
    async.series([
      function (cb) {
        mkdirp(installDir, cb);
      },
      function (cb) {
        that.emit('log', 'Downloading ChromeDriver...');
      ...
      function (cb) {
        that.emit('log', 'Extracting ' + download + '...');
...
SeleniumServer.prototype._installSeleniumServer = function (cb) {
  ...
  fs.exists(jar, function (exists) {
    if (exists) { return cb(); }
    that.emit('log', 'Selenium server is not installed.');
    that.emit('log', 'Downloading selenium server...');
