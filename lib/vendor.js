...

  {
    ...
    checksum: function () {
      ...
      if (platform === 'linux' && arch === 'x64') {
        sum = '832f6a852b6d476cd44f88dc55a0f4afe53aa388';
      } else if (platform === 'linux' && arch === 'x32') {
        sum = '03ec6fa226923094fee7fa8063b7af7cf6935b0f';
      } else if (platform === 'darwin') {
        sum = '5087d28743c01a0d0076be474c06ec21bfb6ef67';
      } else if (platform === 'win32') {
        // chromedriver.exe
        sum = '7aef9f114ef16e8cd6e95095193426c3afcd71a1';
      }
      return sum;
    },
    executable: true,
...
function download(dep, cb) {
  ...
  req.on('response', function (res) {
    ...
    var out;
    if (ext === 'zip') {
      out = unzip.Extract({ path: dep.path });
    } else {
      out = fs.createWriteStream(path.join(dep.path, fname));
    }
    out.on('close', cb);
